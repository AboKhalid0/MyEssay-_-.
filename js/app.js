// app.js — مكتبة | Full Application Logic v4
...
function setupDocLoader() {
  ...
  $('fileInput').addEventListener('change', async (e) => {
    ...
    hide($('viewerEmpty'));
    await loadFile(file);
    // Notify members (can't share file bytes, but tell them what to upload)
    if (firebaseReady && S.room?.id) {
      dbSet('rooms/' + S.room.id + '/sharedDoc', {
        type: 'upload', name: file.name, url: null,
...
function showDocSharedBanner(docInfo) {
  ...
  if (docInfo.type === 'url' && docInfo.url) {
    ...
    banner.innerHTML = `
      <span>📁 <strong>${docInfo.byName}</strong> فتح ملفاً: <em>${docInfo.name}</em> — ارفع نفس الملف</span>
      <button class="btn-primary sm" id="btnAcceptUpload">رفع</button>
      <button class="btn-ghost sm" id="btnDismissBanner">✕</button>`;
    $('viewerArea').prepend(banner);
    document.getElementById('btnAcceptUpload').onclick = () => { banner.remove(); $('fileInput').click(); };
  }
