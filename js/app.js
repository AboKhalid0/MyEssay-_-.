...
// Which overlay is "active" for the current doc type
function activeOverlay() {
  if (!S.doc) return null;
  const t = S.doc.type;
  return {
    wrap:      t==='manga' ? document.getElementById('mangaCanvasWrap')
             : t==='epub'  ? document.getElementById('epubCanvasWrap')
             :               document.getElementById('pdfCanvasWrap'),
    annLayer:  t==='manga' ? document.getElementById('mangaAnnLayer')
             : t==='epub'  ? document.getElementById('epubAnnLayer')
             :               document.getElementById('annLayer'),
    drawCanvas:t==='manga' ? document.getElementById('mangaDrawCanvas')
             : t==='epub'  ? document.getElementById('epubDrawCanvas')
             :               document.getElementById('drawCanvas'),
    overlayRoot:t==='manga'? document.getElementById('mangaOverlayRoot')
             : t==='epub'  ? document.getElementById('epubOverlayRoot')
...
let fbMod = null, firebaseReady = false;
...
const dbSet    = (p,v) => firebaseReady && fbSet(fbRef(p), v);
const dbUpdate = (p,v) => firebaseReady && fbUpdate(fbRef(p), v);
const dbPush   = (p,v) => firebaseReady && fbPush(fbRef(p), v);
const dbRemove = p     => firebaseReady && fbRemove(fbRef(p));

function dbListen(path, cb) {
  if (!firebaseReady) return;
  const r = fbRef(path);
  fbOnValue(r, snap => cb(snap.val()));
  S.dbListeners.push(r);
}
...
function initLobby() {
  ...
  $('btnSaveConfig').addEventListener('click', () => {
    const cfg = { apiKey:$('fbApiKey').value.trim(), projectId:$('fbProjectId').value.trim(),
      databaseURL:$('fbDbUrl').value.trim(), authDomain:$('fbProjectId').value.trim()+'.firebaseapp.com',
    ...
    saveConfig(cfg); toast('تم حفظ إعداد Firebase ✓','success'); $('configForm').classList.add('hidden');
  });
...
async function joinRoom() {
  ...
  try {
    ...
    if (codeSnap.exists()) {
      roomId = codeSnap.val();
      const rs = await fbMod.get(fbRef('rooms/'+roomId));
      if (!rs.exists()){ toast('الغرفة غير موجودة','error'); return; }
      roomData = rs.val();
    } else {
      const all = await fbMod.get(fbRef('rooms'));
...
function enterRoom() {
  ...
  if (firebaseReady) {
    ...
    fbSet(pr, true); fbOnDisconnect(pr).set(false);
  }
...
function setupRoomListeners() {
  ...
  // Annotations — full real-time sync
  ...
  dbListen('rooms/'+S.room.id+'/syncPing', ts => {
    ...
    fbMod.get(fbRef('rooms/'+S.room.id+'/hostPage')).then(s=>{const p=s.val();if(p&&p!==S.currentPage)goToPage(p,false);});
  });
...
// Called after any doc finishes loading
function onDocLoaded() {
  show($('drawToolbar'));
  updateProgress(); saveProgress();
  renderAnnotationsOnPage();
  syncOverlaySize();
  setTimeout(syncOverlaySize, 300); // after layout settles
}

// Make the overlay canvas match the doc content size exactly
function syncOverlaySize() {
  ...
  // Size the overlay root exactly over the content
  ...
  // Match the draw canvas pixel dimensions
  ...
  if (dc.width !== Math.round(rect.width) || dc.height !== Math.round(rect.height)) {
    ...
    dc.width  = Math.round(rect.width);
    dc.height = Math.round(rect.height);
    replayDrawings();
  }
}
...
function initDrawToolbar() {
  ...
  document.querySelectorAll('.dtb-btn[data-tool]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active');
      updateDrawCursor();
      // sticky and text are handled on click, not drag
    });
  });
  ...
  // Attach events to ALL overlay canvases
...
function updateDrawCursor() {
  ...
  // Enable pointer events only when not in "select" mode
...
function onDrawStart(e, cv) {
  ...
  // Special click-only tools
  if (S.tool==='text') {
    S.pendingSticky={xPct:x,yPct:y};
    positionPopup($('textInputPopup'), e.clientX||px, e.clientY||py);
    show($('textInputPopup'));
    $('textInputArea').focus();
    return;
  }
  if (S.tool==='sticky') {
    S.pendingSticky={xPct:x,yPct:y};
    positionPopup($('stickyEditor'), e.clientX||px, e.clientY||py);
    show($('stickyEditor'));
    $('stickyText').focus();
    return;
  }
...
function onDrawMove(e, cv) {
  ...
  if (S.tool==='pen'||S.tool==='marker') {
    ...
    // Also erase annotations visually (they get re-rendered on next page turn)
  } else {
    // Shape preview: redraw everything, then draw shape
    redrawCanvas(cv);
...
function onDrawEnd(e, cv) {
  ...
  if (S.tool==='eraser') {
    // Save erase as annotation with type eraser
    const ann={
  ...
  if (S.tool==='pen'||S.tool==='marker') {
    ...
    const ann={
      id:genId(), type:'stroke',
      subtype: S.tool,
      page:S.currentPage,
      path:S.drawPath,
      color:S.color, size:S.size,
      opacity:S.opacity,
      marker:S.tool==='marker',
...
// ── RENDER ALL ANNOTATIONS ON PAGE ────────────────────────
function renderAnnotationsOnPage() {
  const ov=activeOverlay(); if(!ov)return;

  // 1. Redraw canvas strokes
  redrawCanvas(ov.drawCanvas);

  // 2. Render DOM annotations (sticky, textbox, note dots)
  ov.annLayer.innerHTML='';
  const cw=ov.drawCanvas.width, ch=ov.drawCanvas.height;
...
async function boot() {
  ...
  // Sync overlay size on window resize
  ...
  $('viewerArea').addEventListener('touchend', e=>{
    ...
    // only navigate if not in drawing mode
    if(S.tool!=='select')return;
